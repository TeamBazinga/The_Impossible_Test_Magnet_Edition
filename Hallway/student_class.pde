class Student {
  ...
  void touch(Teacher t) {
    ...
    if (dist(loc.x, loc.y, t.loc2.x, tlth)< 20 || dist(loc.x, loc.y, t.loc1.x, (tlth)-o)< 20 || dist(loc.x, loc.y, rdrw, t.loc2.y)< 20 || dist(loc.x, loc.y, rdrnw, t.loc2.x)< 20 || dist(loc.x, loc.y, rdrnw-o, t.loc2.y)< 20) {
    mainscreen = true;
    }
    if(loc.y>width-100 && loc.x<width/2){
     gamelevel++; 
    }
  }
}
